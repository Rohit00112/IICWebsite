...
const committeeRows = [
  {
    sn: '01',
    role: 'Chairperson, RMC',
    position: 'Academic leadership representative',
    responsibility: 'Research governance and strategic direction',
    remarks: 'Convener',
  },
  {
    sn: '02',
    role: 'Member Secretary',
    position: 'Academic coordination representative',
    responsibility: 'Documentation, meeting records, and reporting',
    remarks: 'Secretariat',
  },
  {
    sn: '03',
    role: 'Computing Faculty Representative',
    position: 'Faculty member',
    responsibility: 'Technology research, innovation, and student projects',
    remarks: 'Member',
  },
  {
    sn: '04',
    role: 'Business Faculty Representative',
    position: 'Faculty member',
    responsibility: 'Management research, enterprise, and applied studies',
    remarks: 'Member',
  },
  {
    sn: '05',
    role: 'Quality and Assessment Representative',
    position: 'Academic quality support',
    responsibility: 'Ethics, compliance, review cycles, and quality records',
    remarks: 'Member',
  },
];
  ...
  return (
    <>
      <div className="hidden overflow-hidden border border-[#e0e6ef] bg-white shadow-[0_20px_60px_rgba(22,36,60,0.08)] md:block">
        <div className="bg-[#A0191D] px-6 py-6 text-white">
        ...
        <table className="w-full border-collapse text-left">
          ...
          <thead className="bg-[#17366F] text-white">
            <tr>
              <th className="w-[76px] px-5 py-5 text-xs font-black uppercase tracking-[0.12em]">SN</th>
              <th className="px-5 py-5 text-xs font-black uppercase tracking-[0.12em]">Committee Role</th>
              <th className="px-5 py-5 text-xs font-black uppercase tracking-[0.12em]">Current Academic Position/Rank</th>
              <th className="px-5 py-5 text-xs font-black uppercase tracking-[0.12em]">Primary Responsibility</th>
          ...
          <tbody>
            {committeeRows.map((row) => (
              <tr key={row.sn} className="border-b border-[#e8edf4] last:border-b-0">
                <td className="px-5 py-6 text-sm font-black text-[#A0191D]">{row.sn}</td>
                <td className="px-5 py-6 text-sm font-black text-[#172033]">{row.role}</td>
                <td className="px-5 py-6 text-sm font-semibold leading-relaxed text-[#64748b]">{row.position}</td>
                <td className="px-5 py-6 text-sm font-semibold leading-relaxed text-[#4f5f75]">{row.responsibility}</td>
      ...
      <div className="grid gap-4 md:hidden">
        <div className="bg-[#A0191D] px-5 py-6 text-white">
          <p className="text-xs font-black uppercase tracking-[0.24em] text-white/72">RMC</p>
        ...
        {committeeRows.map((row) => (
          <article key={row.sn} className="border border-[#e0e6ef] bg-white p-5 shadow-[0_14px_34px_rgba(22,36,60,0.07)]">
            <div className="flex items-start justify-between gap-4">
              <span className="grid h-10 w-10 shrink-0 place-items-center bg-[#A0191D] text-sm font-black text-white">
                {row.sn}
              </span>
            ...
            <h2 className="mt-5 text-xl font-black text-[#172033]">{row.role}</h2>
            <p className="mt-3 text-sm font-semibold leading-relaxed text-[#64748b]">{row.position}</p>
            <p className="mt-2 text-sm font-semibold leading-relaxed text-[#4f5f75]">{row.responsibility}</p>
          </article>
        ))}
      </div>
    </>
...
function HeroSection() {
  return (
    <section className="relative isolate min-h-[70svh] overflow-hidden bg-[#071633]">
      <Image
        ...
        sizes="100vw"
      />
      <div className="absolute inset-0 bg-[#071633]/70" />
      <div className="absolute inset-0 bg-gradient-to-r from-[#071633]/96 via-[#071633]/72 to-[#071633]/22" />
      <div
        className="absolute bottom-0 right-0 h-28 w-[52vw] bg-white"
        style={{ clipPath: 'polygon(18% 42%, 100% 0, 100% 100%, 0 100%)' }}
        aria-hidden="true"
      />
      <div
        className="absolute left-0 top-24 h-28 w-52 bg-[#A0191D]"
...
function ResearchEcosystemCta() {
  return (
    <section className="relative overflow-hidden bg-[#f5f7fa] px-5 py-20 text-center sm:px-8 md:py-28">
      <div
        className="absolute left-[-130px] top-14 h-72 w-96 bg-[#A0191D]"
        style={{ clipPath: 'polygon(0 10%, 74% 0, 100% 50%, 74% 100%, 0 90%, 22% 50%)' }}
        aria-hidden="true"
      />
      <div
        className="absolute bottom-10 right-[-120px] h-28 w-72 bg-[#21409A]/10"
        style={{ clipPath: 'polygon(0 0, 82% 0, 100% 50%, 82% 100%, 0 100%, 14% 50%)' }}
        aria-hidden="true"
      />
      ...
      <div className="relative z-10 mx-auto max-w-5xl">
        ...
        <div className="mt-12 flex flex-wrap items-center justify-center gap-8">
          ...
          <div className="h-12 w-px bg-[#cdd6e4]" aria-hidden="true" />
          <div className="relative h-14 w-28">
            <Image
              src="/images/common/ing.png"
              alt="Innovate Nepal Group"
              fill
              className="object-contain"
              sizes="112px"
            />
        ...
        <Link
          href="/admissions"
          className="mt-12 inline-flex w-full items-center justify-center gap-3 bg-[#A0191D] px-10 py-4 text-sm font-black text-white transition hover:bg-[#861417] sm:w-auto"
        >
          Apply Now

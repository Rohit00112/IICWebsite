      ...
      <div className="max-w-7xl mx-auto px-6 flex flex-col items-center">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 w-full mb-20 max-w-[1440px]">
          {programmes.map((prog, index) => (
            <Tilt key={index} strength={5}>
              <motion.div
                initial={{ opacity: 0, scale: 0.95 }}
                whileInView={{ opacity: 1, scale: 1 }}
                viewport={{ once: true }}
                transition={{ delay: index * 0.2, duration: 1 }}
                className={`${prog.bgColor} rounded-[32px] md:rounded-[40px] p-8 md:p-10 relative overflow-hidden flex flex-col justify-between min-h-[400px] md:h-[440px] shadow-2xl group transition-all duration-500 hover:scale-[1.01] cursor-pointer`}
              >

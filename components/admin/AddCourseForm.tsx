...
export default function AddCourseForm() {
  ...
  const addYear = () => {
    setFormData({
      ...formData,
      curriculum: [...formData.curriculum, { title: 'Year ' + (formData.curriculum.length + 1), modules: [] }]
    });
  };
  ...
  return (
    <div className="max-w-[1600px] mx-auto pb-20">
      <div className="mb-12 flex items-end justify-between border-b border-gray-100 pb-10">
        <div>
          <h1 className="text-5xl font-black text-[#1A2B56] font-sora tracking-tight mb-3">Add New Course</h1>
          <p className="text-gray-500 font-medium text-lg">Initialize a new academic program with full curriculum details.</p>
        </div>
          ...
          <div className="bg-white p-12 rounded-[40px] border border-gray-100 shadow-sm">
            ...
            {formData.curriculum.length === 0 ? (
              <div className="py-20 text-center border-2 border-dashed border-gray-100 rounded-[32px]">
                <p className="text-gray-400 font-medium mb-6">No curriculum structure defined yet.</p>
                <button 
                  type="button"
                  onClick={addYear}
                  className="px-8 py-4 bg-[#21409A] text-white rounded-2xl font-bold text-sm"
                >
                  Create First Year
                </button>
              </div>
